...
name="comp_square_plate_wNotch_NoOP_EvenMoreRefined4LessTaper_NoCrust_NoSidePlate_25kmCoreC_DoubleSlab_StiffEnd"
...
name2="temp_square_plate_wNotch_NoOP_EvenMoreRefined4LessTaper_NoCrust_NoSidePlate_25kmCoreC_DoubleSlab_StiffEnd"
...
tres1  =  np.linspace(tmin,0.5*tmax - 0.54*width_omega,int((tnum-num_t_refine)/6)+1)
tres2  =  np.linspace(0.5*tmax - 0.54*width_omega,0.5*tmax - 0.46*width_omega,int(num_t_refine/2)+1)
tres3  =  np.linspace(0.5*tmax - 0.46*width_omega,0.5*tmax + 0.46*width_omega,int(4*(tnum-num_t_refine)/6)+1)
tres4  =  np.linspace(0.5*tmax + 0.46*width_omega,0.5*tmax + 0.54*width_omega,int(num_t_refine/2)+1)
tres5  =  np.linspace(0.5*tmax + 0.54*width_omega,tmax,int((tnum-num_t_refine)/6)+1)
...
num_p_refine = 120
pres1 = np.linspace(pmin,1.2*notch_omega,int(2*(num_p_refine/6))+1)
pres2 = np.linspace(1.2*notch_omega, pmax - (length_omega + 1.2*notch_omega),int(4*(pnum-num_p_refine)/6)+1)
pres3 = np.linspace(pmax - (length_omega + 1.2*notch_omega),pmax - (length_omega - 0.4*notch_omega),int(3*(num_p_refine/6))+1)
pres4 = np.linspace(pmax - (length_omega - 0.4*notch_omega),pmax-(0.4*notch_omega),int(2*(pnum-num_p_refine)/6)+1)
pres5 = np.linspace(pmax-(0.4*notch_omega),pmax,int(num_p_refine/6)+1)
pvals = np.concatenate((pres1,pres2[1:],pres3[1:],pres4[1:],pres5[1:]), axis=0)

# pres1 = np.linspace(pmin,1.2*notch_omega,int(3*(num_p_refine/6))+1)
# pres2 = np.linspace(1.2*notch_omega, pmax - (1.05*length_omega),int(4*(pnum-num_p_refine)/6)+1)
# pres3 = np.linspace(pmax - (1.05*length_omega),pmax - (0.95*length_omega),int(2*(num_p_refine/6))+1)
# pres4 = np.linspace(pmax - (0.95*length_omega),pmax-(0.4*notch_omega),int(2*(pnum-num_p_refine)/6)+1)
# pres5 = np.linspace(pmax-(0.4*notch_omega),pmax,int(num_p_refine/6)+1)
# pvals = np.concatenate((pres1,pres2[1:],pres3[1:],pres4[1:],pres5[1:]), axis=0)
...
for t in range(tnum + 1):
    for p in range(pnum + 1):
            for r in range(rnum + 1):
                ...
                if theta >= (0.5*np.pi - 0.5*width_omega) and theta <= (0.5*np.pi + 0.5*width_omega) and phi >= (pmax - length_omega): 
                ...
                # elif theta >= (0.5*np.pi - 0.5*width_omega) and theta <= (0.5*np.pi + 0.5*width_omega) and phi >= ((pmax - 0.5*length_omega)-0.5*stiff_edge_length) and phi <= ((pmax - 0.5*length_omega)+0.5*stiff_edge_length):

                #     if rad >= (rmax - lith_thick):
                #         C[ind,4]=1

                # elif theta <= (0.5*np.pi + 0.5*width_omega) and theta <= (0.5*np.pi + 0.5*width_omega) and phi > ((pmax - 0.5*length_omega)+0.5*stiff_edge_length):
                ...
                # notch 2
                elif theta >= (0.5*np.pi - 0.5*width_omega) and theta <= (0.5*np.pi + 0.5*width_omega) and phi >= (pmax - length_omega - notch_omega) and phi < (pmax - length_omega):
                    z1 = notch_radius;
                    z  = rmax - rad
                    dx = ((pmax - length_omega)-(phi)) * rmax
